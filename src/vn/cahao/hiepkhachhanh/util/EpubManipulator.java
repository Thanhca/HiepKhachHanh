...
package vn.cahao.hiepkhachhanh.util;
...
public class EpubManipulator {
	//capt
	//doi ten de ko lap lai
	static String inputName=Common.KEY_NAME_APP;//RamdomToken
	public static String NaneTruyen ="epubtemp/"+ inputName +"/";
	public static boolean checkLinkHtml=false;
	//----------------
...
//+ "/epubtemp/phanthanhca/"; //
	...
	public String goToPreviousChapter() throws Exception {
		//--capt
		if(checkLinkHtml){
			templink=currentSpineElementIndex+1;
			currentSpineElementIndex=templink;
			checkLinkHtml=false;
		}
		//------------------
		return goToPage(this.currentSpineElementIndex - 1);
	...
	public void createTocFile() {
		...
		String html = "<html><meta http-equiv='Content-Type' content='text/html;charset=UTF-8'><body><ul>";
		
		tmp = toc.getTocReferences();

		if (tmp.size() > 0) {
			//html += getS(R.string.tocReference);
			//html +="<h1>" + "Rao vat-->cahao.vn"  +"</h1>";
		...
		try {
			...
			fw.flush();
			fw.close();
			Log.d("location + decompressedFolder +/Toc.html===>:", ""+file);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	//--capt
	String file_toc=null;
	String file_temphtml=null;
	String file_temphtml1=null;
	int templink=1;
	//------------
	...
	public boolean goToPage(String page) {
		...
		//NaneTruyen
		String temptruyen=NaneTruyen + "0/";
		for (String retval: page.split(temptruyen)){
			//file_toc=retval;
			file_toc="/Ngon_tinh_An-_em_trong_hi_c_split_001.html";
			Log.d("capt-file_toc->page.split():", ""+retval);
	     }
		//-capt
		/*
		for (String retval: page.split("epubtemp/0/")){
			file_toc=retval;
			Log.d("capt-file_toc->page.split():", ""+retval);
	     }
		*/
		if(file_toc.equalsIgnoreCase("Toc.html")){
			Log.d("capt-->page.split():", "ok");
		} else if(file_toc.equalsIgnoreCase("/titlepage.xhtml")){
			
		}
		else{
		
			 file_temphtml=file_toc.substring(file_toc.lastIndexOf("_"));			
			////////file_temphtml1=file_temphtml.substring(file_temphtml.lastIndexOf("_"));
			for (String retval: file_temphtml.split("_")){
				file_temphtml1=retval;
		    }

			for (String retval1: file_temphtml1.split(".html")){
				currentSpineElementIndex=Integer.valueOf(retval1);
				Log.d("capt-->lastIndexOf",""+ currentSpineElementIndex);
		    }
